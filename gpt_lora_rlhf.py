...
from trl import PPOConfig, PPOTrainer, create_reference_model, set_seed, AutoModelForSeq2SeqLMWithValueHead, AutoModelForCausalLMWithValueHead
...
class ScriptArguments:
    ...
    model_name: Optional[str] = field(default="heegyu/WizardVicuna-pythia-1.4b-deduped", metadata={"help": "the model name"})
    ...
    model_save_path: Optional[str] = field(
        default="./checkpoint/wizvic-1.4b-rlhf",
...
def build_dataset(
    ...
    ds = ds.map(tokenize, batched=False, num_proc=8)
...
for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader), desc=config.model_name, position=0, leave=False):
